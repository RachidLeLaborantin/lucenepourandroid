...
public class Searcher implements AutoCloseable {
    ...
    SearchResult searchAfter(ScoreDoc lastScoreDoc, Query query, Sort sort, int maxCount)
        ...
        for (int i = 0; i < topDocsLen; i++) {
            ...
            Documents doc = Indexer.fromLuceneDocument(luceneDoc);
            docs.add(doc);
        }

        return new SearchResult(topDocs.totalHits, docs,
                nextSearchAfterDoc, query, sort, highlightingHelper);
    }
    ...
    public enum SortBy {
        ...
        YEAR(new Sort(new SortField(Indexer.YEAR_FIELD_NAME, SortField.Type.STRING, true))),
        RATING(new Sort(new SortField(Indexer.RATING_FIELD_NAME, SortField.Type.STRING, true)));
